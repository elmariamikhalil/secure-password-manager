  ...
  try {
    ...
    if (data.encryptionParams) {
      ...
      chrome.storage.session.set({
        encryptionKey: arrayBufferToBase64(await window.crypto.subtle.exportKey('raw', encryptionKey))
      });
    }
...
async function captureCredential(credential) {
  try {
    ...
    if (existingCredentials.success && existingCredentials.credentials) {
      ...
      const duplicate = existingCredentials.credentials.find(cred => 
...
chrome.notifications.onButtonClicked.addListener(async (notificationId, buttonIndex) => {
  if (buttonIndex === 0) { // First button (Save/Update)
    const { pendingCredential, pendingCredentialUpdate } = 
      await chrome.storage.local.get(['pendingCredential', 'pendingCredentialUpdate']);
    ...
    // User clicked "Ignore"
    chrome.storage.local.remove(['pendingCredential', 'pendingCredentialUpdate']);
  }
